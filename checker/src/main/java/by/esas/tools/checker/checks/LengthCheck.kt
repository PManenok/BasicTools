...
class LengthCheck : BaseCheck {
    ...
    constructor(min: Int = 0, max: Int = Int.MAX_VALUE, @StringRes errorRes: Int, vararg params: Any) : super(errorRes, params) {
        this.min = min
        this.max = max
    }

    constructor(min: Int = 0, max: Int = Int.MAX_VALUE, errorMessage: String = "Length must be from $min to $max") : super(errorMessage) {
        this.min = min
        this.max = max
    }
